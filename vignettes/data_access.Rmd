...
For reproducibility and fast vignette builds, we use a shipped global measles
example panel and pinned fixtures.
...
```{r}
cache_dir <- tools::R_user_dir("vpdsus", which = "cache")
cache_files <- if (dir.exists(cache_dir)) list.files(cache_dir, recursive = TRUE, full.names = TRUE) else character()
tibble::tibble(
  cache_dir = cache_dir,
  n_files = length(cache_files),
  bytes = if (length(cache_files)) sum(file.info(cache_files)$size, na.rm = TRUE) else 0
)
```
...
### Build a modelling-ready panel

Once you have standardised coverage, cases, and demography series you can join
these into a country-year panel for downstream modelling:

```{r}
demog <- get_demography(source = "fixture_wpp", years = 2020, countries = "USA")

panel_usa_2020 <- build_panel(
  coverage = cov,
  cases = cases,
  demography = demog
)
...
panel_usa_2020
panel_diagnostics(panel_usa_2020)
```

### Where to go next (end-to-end workflow)

The functions above focus on data retrieval and standardisation. A typical end-to-end
analysis then:

1. builds a modelling-ready panel (coverage + cases + demography),
2. estimates susceptibility, and
3. produces a WHO-style risk ranking (and optionally fits outbreak models).

This repository ships `vpdsus_example_panel()` as a global measles panel so the
workflow can be demonstrated at realistic scale without live downloads:
